...
(function () {
  ...
  // Atmosphère Fresnel
  earthGroup.add(new T.Mesh(new T.SphereGeometry(5.22, 64, 64), new T.ShaderMaterial({
    transparent: true, side: T.FrontSide, blending: T.AdditiveBlending, depthWrite: false,
    ...
    fragmentShader: `uniform vec3 uSun; varying vec3 vN,vW; void main(){ vec3 v=normalize(cameraPosition-vW); float f=pow(1.0-dot(vN,v),4.0); float s=max(dot(vN,normalize(uSun)),0.0); vec3 c=mix(vec3(0.08,0.18,0.55),vec3(0.25,0.50,0.90),s); gl_FragColor=vec4(c,f*0.65); }`,
  })));

  // Halo externe
  earthGroup.add(new T.Mesh(new T.SphereGeometry(5.8, 48, 48), new T.ShaderMaterial({
    transparent: true, side: T.BackSide, blending: T.AdditiveBlending, depthWrite: false,
    vertexShader: `varying vec3 vN; void main(){ vN=normalize(normalMatrix*normal); gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0); }`,
    fragmentShader: `varying vec3 vN; void main(){ float i=pow(0.6-dot(vN,vec3(0,0,1)),3.5); gl_FragColor=vec4(0.12,0.30,0.65,i*0.35); }`,
  })));

  // ── ORBITES & SATELLITES ──────────────────────────────────
  const ORBITS = [
    { r: 7.5,  inc: 97.4, speed: 0.0040, phase:   0 },
    { r: 8.4,  inc: 97.4, speed: 0.0031, phase: 130 },
    { r: 7.0,  inc: 51.6, speed: 0.0022, phase:  60 },
    { r: 9.0,  inc: 97.8, speed: 0.0048, phase: 240 },
  ];
  ...
  function makeSat() {
    const g = new T.Group();
    g.add(new T.Mesh(new T.BoxGeometry(0.20, 0.20, 0.34),
    ...
    const ant = new T.Mesh(new T.CylinderGeometry(0.006, 0.006, 0.18, 5),
      new T.MeshBasicMaterial({ color: 0xe0e4f0 }));
    ant.position.y = 0.18; g.add(ant);
    return g;
  }

  const TRAIL_LEN = 70;
  const tmpV = new T.Vector3(), tmpEul = new T.Euler();
  ...
  const satellites = ORBITS.map(o => {
    ...
    return { sat, geo, tPos, tCol, buf: new Float32Array(TRAIL_LEN * 3),
      head: 0, count: 0, ...o, incRad: o.inc * Math.PI / 180, angle: o.phase * Math.PI / 180 };
  });
  ...
  function animate() {
    ...
    satellites.forEach(sd => {
      ...
      tmpV.set(sd.r * Math.cos(sd.angle + 0.02), 0, sd.r * Math.sin(sd.angle + 0.02)).applyEuler(tmpEul);
      sd.sat.lookAt(tmpV);

      const h = sd.head % TRAIL_LEN;
      sd.buf[h*3] = sd.sat.position.x; sd.buf[h*3+1] = sd.sat.position.y; sd.buf[h*3+2] = sd.sat.position.z;
      sd.head++; sd.count = Math.min(sd.count + 1, TRAIL_LEN);

      const pA = sd.geo.attributes.position, cA = sd.geo.attributes.color;
      for (let i = 0; i < sd.count; i++) {
        const idx  = ((sd.head - 1 - i + TRAIL_LEN * 100) % TRAIL_LEN) * 3;
        const fade = (1 - i / sd.count) * 0.5;
        pA.setXYZ(i, sd.buf[idx], sd.buf[idx+1], sd.buf[idx+2]);
        cA.setXYZ(i, fade, fade, fade);
      }
